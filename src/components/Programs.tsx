...
export function Programs() {
  ...
  function handleWaitlist(e: FormEvent) {
    ...
    // TODO: Replace with your email service (Mailchimp, Formspree, Supabase, etc.)
    // For now, stores locally and opens mailto as fallback
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-14">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight mb-4">
            {t.programs.heading.before}
            <span className="gradient-text">{t.programs.heading.highlight}</span>
          </h2>
        ...
        <div className="grid lg:grid-cols-2 gap-6 lg:gap-8">
          {/* K-12 AI Curriculum */}
          <div className="relative bg-white rounded-2xl border border-gray-100 shadow-lg shadow-gray-100/60 overflow-hidden group hover:shadow-xl hover:shadow-gray-200/60 transition-all duration-300">
            {/* Top accent bar */}
            ...
            <div className="p-6 sm:p-8">
              ...
              <div className="flex items-center justify-between mb-5">
                ...
                <span className="inline-flex items-center gap-1.5 bg-green-100 text-green-700 text-xs font-semibold px-3 py-1 rounded-full">
                  ...
                  {t.programs.k12.badge}
                </span>
              </div>

              {/* Title */}
              <h3 className="text-xl sm:text-2xl font-bold text-text mb-1">
                {t.programs.k12.title}
              </h3>
              <p className="text-sm text-primary font-medium mb-4">
                {t.programs.k12.subtitle}
              </p>

              {/* Description */}
              <p className="text-sm text-text-muted leading-relaxed mb-6">
                {t.programs.k12.description}
              </p>
              ...
              <div className="space-y-4 mb-6">
                <div className="bg-gray-50 rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <span className="text-xs font-semibold text-text uppercase tracking-wide">{t.programs.k12.certifiedBy}</span>
                  </div>
                  <p className="text-sm font-medium text-text">{t.programs.k12.certifier}</p>
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-gray-50 rounded-xl p-4">
                    <div className="text-xs font-semibold text-text-muted uppercase tracking-wide mb-1">{t.programs.k12.grades}</div>
                    <p className="text-sm font-medium text-text">{t.programs.k12.gradesValue}</p>
                  </div>
                  <div className="bg-gray-50 rounded-xl p-4">
                    <div className="text-xs font-semibold text-text-muted uppercase tracking-wide mb-1">{t.programs.k12.domains}</div>
                    <p className="text-sm font-medium text-text">{t.programs.k12.domainsValue}</p>
                  </div>
                </div>
              </div>
            ...
            <div className="p-6 sm:p-8">
              ...
              <div className="flex items-center justify-between mb-5">
                ...
                <span className="inline-flex items-center gap-1.5 bg-violet-100 text-violet-700 text-xs font-semibold px-3 py-1 rounded-full">
                  ...
                  {t.programs.university.badge}
                </span>
              </div>

              {/* Title */}
              <h3 className="text-xl sm:text-2xl font-bold text-text mb-1">
                {t.programs.university.title}
              </h3>
              <p className="text-sm text-violet-600 font-medium mb-4">
                {t.programs.university.subtitle}
              </p>

              {/* Description */}
              <p className="text-sm text-text-muted leading-relaxed mb-6">
                {t.programs.university.description}
              </p>

              {/* Feature list */}
              <div className="space-y-3 mb-6">
                {t.programs.university.features.map((feature, i) => (
              ...
              {submitted ? (
                <div className="flex items-center gap-3 bg-violet-50 border border-violet-200 rounded-xl px-5 py-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-violet-800">{t.programs.university.subscribed}</p>
                    <p className="text-xs text-violet-600">{t.programs.university.subscribedMsg}</p>
                  </div>
                </div>
              ) : (
                <form onSubmit={handleWaitlist} className="flex gap-2">
                  <input
                    ...
                    placeholder={t.programs.university.emailPlaceholder}
                  ...
                  <button
                    ...
                    <span className="hidden sm:inline">{t.programs.university.cta}</span>
                  </button>
                </form>
              )}
            </div>
